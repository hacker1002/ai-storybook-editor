// sound-item.tsx - Single stage sound item with header (actions) and audio player or empty message
...
const log = createLogger('Editor', 'StageSoundItem');
...
export function StageSoundItem({ stageKey, sound, onBrowse }: StageSoundItemProps) {
  ...
  const handleFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { publicUrl } = await uploadAudioToStorage(file, `stages/${stageKey}/sounds`);
      log.info('handleUpload', 'success', { publicUrl });
      updateStageSound(stageKey, sound.key, { media_url: publicUrl });
