...
import { useThemes, useSelectedThemeIds, useThemeActions } from '@/stores/theme-store';
import { useGenres, useSelectedGenreIds, useGenreActions } from '@/stores/genre-store';
...
export function ConfigGeneralSettings() {
  ...
  React.useEffect(() => {
    if (book?.artstyle_id && !artStyleName) {
      log.debug('fetchArtStyle', 'triggering', { artStyleId: book.artstyle_id });
  ...
  const dimensionLabel = book.dimension != null ? (DIMENSION_MAP[book.dimension as keyof typeof DIMENSION_MAP] ?? '—') : '—';
  const audienceLabel = book.target_audience != null ? (TARGET_AUDIENCE_MAP[book.target_audience as keyof typeof TARGET_AUDIENCE_MAP] ?? '—') : '—';

  const eraOptions = eras.map((e) => ({ value: e.id, label: e.name }));
  const locationOptions = locations.map((l) => ({ value: l.id, label: l.name }));
  const languageOptions = SUPPORTED_LANGUAGES.map((l) => ({ value: l.code, label: l.label }));
  ...
  const handleLanguageChange = (value: string) => {
    log.info('handleLanguageChange', 'updating', { language: value });
    void updateBook(book.id, { original_language: value });
  };
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      ...
      {/* ORIGINAL LANGUAGE — single-select, editable */}
      <div>
